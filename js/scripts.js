...
const tomarDatosCertificado = () => {
  ...
  fechaCertificado.innerHTML = `Córdoba, ${hoy.getDate()} de ${
    meses[hoy.getMonth()]
  } de ${hoy.getFullYear()}`;
  const titulo = document.getElementById("titulo");
  textoPrincipal.innerHTML = `Por la presente certifico que <b>${nombre.toUpperCase()}</b>, DNI ${dni} recibió el sacramento de ${tipo} en esta parroquia, Nuestra Señora de los Ángeles, el día <b> ${fecha.getDate()} de ${
...
const tomarDatosCertificadoAbreviado = () => {
  const hoy = new Date();
  const meses = [
    "enero",
    "febrero",
